...
mCminorWing<-mean(data$d1[1:6])
mCgunlaWing<-mean(data$d1[7:8])

mCminorTail<-mean(data$d1[9:14])
mCgunlaTail<-mean(data$d1[15:16])
...
ICN<-ggplot(data,aes(x=d1,y=Group))+
  ...
  geom_vline(data=subset(data, Measurement=="Wing"),aes(xintercept = mCminorWing),linetype="dotted")+
  geom_vline(data=subset(data, Measurement=="Tail"),aes(xintercept = mCminorTail),linetype="dotted")+
  geom_vline(data=subset(data, Measurement=="Wing"),aes(xintercept = mCgunlaWing),linetype="dashed")+
  geom_vline(data=subset(data, Measurement=="Tail"),aes(xintercept = mCgunlaTail),linetype="dashed")+
  geom_linerange(aes(xmin=CI99low1,xmax=CI99upp1), color="lightgray", size=0.3125)+
  geom_linerange(aes(xmin=CI95low1,xmax=CI95upp1), color="darkgray", size=0.625)+
  geom_linerange(aes(xmin=CI80low1,xmax=CI80upp1), size=1.25)+
...
IAvH<-ggplot(data,aes(x=d2,y=Group))+
  ...
  geom_vline(data=subset(data, Measurement=="Wing"),aes(xintercept = mCgunlaWing),linetype="dashed")+
  geom_vline(data=subset(data, Measurement=="Tail"),aes(xintercept = mCgunlaTail),linetype="dashed")+
  geom_linerange(aes(xmin=CI99low2,xmax=CI99upp2), color="lightgray", size=0.3125)+
  geom_linerange(aes(xmin=CI95low2,xmax=CI95upp2), color="darkgray", size=0.625)+
  geom_linerange(aes(xmin=CI80low2,xmax=CI80upp2), size=1.25)+
  geom_point(shape=15, size=2.5)+
  facet_wrap(~Measurement, ncol=1)+
  labs(x="IAvH-A-96",
       y="")+
